...
void NeuralNetwork::saveNetwork(QString path)
{
    ...
    for ( unsigned i=0; i<m_layers.length(); ++i )
    {
        ...
        for ( unsigned j=0; j<m_layers[i]->length(); ++j )
        {
            ...
            QDomElement biasElement = dom.createElement("Bias");
            QDomElement weightsElement = dom.createElement("Weights");
            QVector<double> weights;

            biasElement.appendChild(dom.createTextNode(QString::number(m_layers[i]->getNeurons()[j]->getBias())));
            ...
            neuronElement.appendChild(biasElement);
...
void NeuralNetwork::loadNetwork(QString path, unsigned inputLayerSize)
{
    QDomDocument dom;
    QDomElement root, layers;
    QFile file(path);
    unsigned layerIdx = 0;
    m_inputLayer = new NeuronsLayer();

    m_inputLayer->createNeurons(inputLayerSize);
    ...
    layers = root.firstChildElement("Layers");
    
    for ( QDomElement layerDomElement=layers.firstChildElement("Layer"); !layerDomElement.isNull(); layerDomElement = layerDomElement.nextSiblingElement("Layer"))
    {
        NeuronsLayer* nnLayer = new NeuronsLayer();
        QDomElement neuronsDomElement = layerDomElement.firstChildElement("Neurons");
        QDomElement neuronsNumberDomElement = layerDomElement.firstChildElement("NeuronsNumber");
        unsigned neuronIdx = 0;

        nnLayer->createNeurons(neuronsNumberDomElement.text().toInt());

        for ( QDomElement neuronDomElement=neuronsDomElement.firstChildElement("Neuron"); !neuronDomElement.isNull(); neuronDomElement=neuronDomElement.nextSiblingElement("Neuron"))
        {
           QDomElement neuronBiasDomElement = neuronDomElement.firstChildElement("Bias");
           QDomElement neuronWeightsDomElement = neuronDomElement.firstChildElement("Weights");
           unsigned weightNumber = 0;

           nnLayer->at(neuronIdx)->setBias(neuronBiasDomElement.text().toDouble());

           for ( QDomElement weightDomElement=neuronWeightsDomElement.firstChildElement("Weight"); !weightDomElement.isNull(); weightDomElement=weightDomElement.nextSiblingElement("Weight"))
           {
               if ( layerIdx == 0 )
                   nnLayer->at(neuronIdx)->setBackwardConnection(m_inputLayer->at(weightNumber++), weightDomElement.text().toDouble());
               else
               {
                   nnLayer->at(neuronIdx)->setBackwardConnection(m_layers.back()->at(weightNumber), weightDomElement.text().toDouble());
                   m_layers.back()->at(weightNumber)->setForwardConnection(nnLayer->at(neuronIdx));
                   ++weightNumber;
               }
           }
